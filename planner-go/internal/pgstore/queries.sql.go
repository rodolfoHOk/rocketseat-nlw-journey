...
func (q *Queries) GetParticipant(ctx context.Context, id uuid.UUID) (GetParticipantRow, error) {
...
func (q *Queries) GetParticipants(ctx context.Context, tripID uuid.UUID) ([]GetParticipantsRow, error) {
...
type GetTripActivitiesRow struct {
	ID       uuid.UUID        `db:"id" json:"id"`
	TripID   uuid.UUID        `db:"trip_id" json:"trip_id"`
	Title    string           `db:"title" json:"title"`
	OccursAt pgtype.Timestamp `db:"occurs_at" json:"occurs_at"`
...
func (q *Queries) GetTripActivities(ctx context.Context, tripID uuid.UUID) ([]GetTripActivitiesRow, error) {
	rows, err := q.db.Query(ctx, getTripActivities, tripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetTripActivitiesRow
	for rows.Next() {
		var i GetTripActivitiesRow
		if err := rows.Scan(
			&i.ID,
			&i.TripID,
			&i.Title,
...
func (q *Queries) GetTripLinks(ctx context.Context, tripID uuid.UUID) ([]GetTripLinksRow, error) {
	rows, err := q.db.Query(ctx, getTripLinks, tripID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetTripLinksRow
	for rows.Next() {
		var i GetTripLinksRow
		if err := rows.Scan(
			&i.ID,
			&i.TripID,
			&i.Title,
