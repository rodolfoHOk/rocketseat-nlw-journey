...
import { participantServer } from '@/server/participant-server';
...
export default function Trip() {
  ...
  async function handleConfirmAttendance() {
    try {
      ...
      await participantServer.confirmTripByParticipantId({
      ...
      >
        <View className="gap-4 mt-4">
          <Text className="text-zinc-400 font-regular leading-6 my-2">
            ...
            nas datas de{' '}
            <Text className="font-semibold text-zinc-100">
              {`${dayjs(tripDetails.starts_at).date()} a ${dayjs(
                tripDetails.ends_at
              ).date()} de ${dayjs(tripDetails.ends_at).format('MMMM')}.\n\n`}
            </Text>
